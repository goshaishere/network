...
- CD: [`.github/workflows/deploy.yml`](../.github/workflows/deploy.yml) — ручной deploy по `workflow_dispatch` (SSH): выставляются **`NETWORK_BACKEND_IMAGE`** / **`NETWORK_FRONTEND_IMAGE`** (`ghcr.io/<owner>/<repo>/backend|frontend:<tag>`), **`docker compose -f docker-compose.stack.yml pull && up -d`**, smoke **`GET /api/v1/health/ready/`** через nginx на `:80`. На сервере в `/opt/network` должен лежать репозиторий (или скопированы) **`docker-compose.stack.yml`** и **`.env`** (см. [`stack.env.example`](../stack.env.example)).
- Локальная разработка: [`docker-compose.yml`](../docker-compose.yml) — bind-mount `backend`, Mailhog, миграции в `command` у `api`.
- Деплой / stage / prod: [`docker-compose.stack.yml`](../docker-compose.stack.yml) — только образы из registry, том Postgres, healthcheck API (**curl** на **`/api/v1/health/ready/`**), **без** монтирования кода.
- Наблюдаемость (фаза 6): [`docker-compose.observability.yml`](../docker-compose.observability.yml) (профиль `observability`), [runbooks/observability.md](./runbooks/observability.md).
