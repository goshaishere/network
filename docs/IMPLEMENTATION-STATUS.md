...
- **Сделано:** `apps.social` — друзья и заявки; **`GET /api/v1/social/feed/`** (`apps/social/feed.py`, offset, **`heapq.merge`**, **`hidden_from_feed=False`**, **`attachment_url`**); **`POST /api/v1/social/reports/`**, модель **`ContentReport`**, `social/admin.py`; стена/сообщества — вложение, **`hidden_from_feed`**, **`GET/PATCH …/communities/<slug>/posts/<id>/`**; фронт: превью и жалоба в **`/feed`**, загрузка картинки на стене и в сообществе, переключатель «скрыть из ленты»; pytest **`tests/test_social_feed.py`** (в т.ч. скрытие из ленты и жалобы).
- **Позже:** пуши, курсорная пагинация без лимита слияния, полноценная модерация жалоб в UI/API.
...
3. Фаза 10+: уведомления (в т.ч. push) и углублённая модерация — по продукту.
