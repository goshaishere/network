...
| 10+ | Соцрасширение | **Частично** | **Лента:** `GET /social/feed/` (стены друзей с публичным профилем + посты из сообществ пользователя), UI **`/feed`** с **QInfiniteScroll**. **Моя страница** **`/me`**, **мои сообщества** **`/communities/mine`**, сайдбар: лента, страница, друзья, сообщения, мои группы. Нет вложений в ленту, пушей, отдельной модерации ленты. |
...
- **Сделано:** `apps.social` — друзья и заявки; **`GET /api/v1/social/feed/`** (`apps/social/feed.py`, offset-пагинация); **`GET /api/v1/communities/mine/`**; фронт **`/feed`**, **`/me`**, **`/communities/mine`**, обновлённый сайдбар и главная; pytest **`tests/test_social_feed.py`**, друзья в `tests/test_pipeline_features.py`.
- **Позже:** вложения в постах ленты, пуши, курсорная пагинация без лимита слияния, модерация.
