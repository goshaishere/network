...
- `docker-compose.yml` — локальный dev (bind-mount `backend`, Mailhog, healthcheck API).  
- `docker-compose.stack.yml` — сервер / stage / prod: образы **`NETWORK_BACKEND_IMAGE`** и **`NETWORK_FRONTEND_IMAGE`** из GHCR, без монтирования кода; пример переменных — `stack.env.example`.  
- `frontend/nginx-default.conf` — SPA + **прокси** `location /api/` и `/ws/` на сервис `api:8000` (один origin с фронтом).  
- `.dockerignore` в `backend/` и `frontend/` — уменьшить контекст сборки.
